...
and
  ...
  | DeRef(e) ->
    ...
    (
      match t1 with 
      | RefType temp1 -> temp1
      | _ -> failwith "wrong type in deref"
    )
  ...
  | SetRef(e1,e2) ->
    ...
    let deRefered = (
      match t1 with 
      | RefType temp1 -> temp1
      | _ -> failwith "Wrong type in setref"
    ) in
    if deRefered = t2 then 
      UnitType 
    else 
      failwith "SetRef has incompatible types"
  ...
  | Unpair(id1, id2, def, body) ->
    let t1 = type_of_expr en def in
    (
      match t1 with 
      | PairType(e1,e2)-> 
          let k = type_of_expr (extend_tenv id1 e1 (extend_tenv id2 e2 en)) body in
          k
      | _ -> failwith "pair type inproper"
    )
  ...
  | Cons(he, te) ->
    ...
    let derefered = (
      match t2 with 
      | ListType e1 -> e1
      | _ -> failwith "Required type list in Cons"
    )
    in
    if t1 = derefered then 
      t2
    else
      failwith "List can have only one type of Data" 
  ...
  | Null(e) ->
    ...
    (
      match t1 with 
      | ListType l1 -> BoolType
      | _ -> failwith "Require Type of List in Null"
    )
  ...
  | Hd(e) ->
    ...
    let de = (
      match t1 with 
      | ListType e1 -> e1
      | _ -> failwith "Required type list in Hd"
    ) in
    de
  ...
  | Node(de, le, re) ->
    ...
    (
      match t2,t3 with
      | TreeType _,TreeType _ -> TreeType(t1)  
      | _ -> failwith "Type incompatible in Node"
    )
  
    
  | NullT(t) ->
    let t1 = type_of_expr en t
    in
    (
      match t1 with 
      | TreeType l1 -> BoolType
      | _ -> failwith "Require Type of Tree in NullT"
    )
  ...
  | GetData(t) ->
    ...
    (
      match t1 with
      | TreeType x -> x
      | _ -> failwith "Wrong Type in GetData in Tree"
    )
  ...
  | GetLST(t) ->
    ...
    (
      match t1 with
      | TreeType x -> TreeType(x)
      | _ -> failwith "Wrong Type in GetLST in Tree"
    )
  ...
  | GetRST(t) ->
    ...
    (
      match t1 with
      | TreeType x -> TreeType(x)
      | _ -> failwith "Wrong Type in GetRST in Tree"
    )
